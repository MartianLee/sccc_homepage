...
*/
  ...
  echo "<table border='1'>
          <tr>
            <td width='20%' align='center'>RANKING</td>
            <td width='30%' align='center'>NAME</td>
            <td width='25%' align='center'>WIN</td>
            <td width='25%' align='center'>LOSE</td>
          </tr>";
  if ($result = mysqli_query($con, "SELECT id,name,win,lose FROM Score ", MYSQLI_USE_RESULT)) {
      // 레코드 출력
      while ($row = mysqli_fetch_object($result)) {
          echo '<tr> <td>'.$row->id.'</td> <td> '.$row->name.' </td> <td> '.$row->win.'</td> <td> '.$row->lose.'</td> </tr>';
      }
